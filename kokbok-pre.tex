\documentclass[12pt,a4paper]{article}
\usepackage[swedish]{babel}
\usepackage[T1]{fontenc}
\usepackage[utf8]{inputenc}
\usepackage{siunitx}[=v2] % xcookybooky uses an older incompatible version
\usepackage{tocloft} % simply including this makes the toc spacing prettier
\usepackage{tabularx}
\let\oldsection\section
\usepackage{titlesec}
\usepackage{xcookybooky}
\usepackage{hyperref}

% versioning %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\input{versions.tex}

% title page %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\title{\Huge Eriks Kokbok\\\medskip
  \large Utgåva \shortVersion\\\smallskip
  \small\texttt\longVersion}
\author{Erik Rimskog}

% SI %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\DeclareSIUnit\litre{l} % recommended way to use a small letter, see the doc
\DeclareSIUnit\liter{l} % but i guess it doesn't work in version 2...
\DeclareSIUnit\tesked{tsk}
\DeclareSIUnit\matsked{msk}
\DeclareSIUnit\kryddsked{krm}
\sisetup{
  range-phrase={\,--\,},
  range-units=single,
}

% fancyhdr %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% NOTE: fix fancyhdr warning the way it suggests
\setlength{\headheight}{15pt}

% xcookybooky %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% NOTE: fix for outdated xcookybooky / updated lettrine
% https://tex.stackexchange.com/a/481705
\renewcommand{\step}
{%
  \stepcounter{step}%shouldn't be in the argument of lettrine
  \lettrine
  [%
  lines=2,
  lhang=0,
  loversize=0.15,
  slope=0em,
  findent=1em,
  nindent=0em
  ]{\thestep}{}%
}

\setHeadlines{
  inghead = Ingredienser,
  portionvalue = Portioner,
  prephead = Förberedelse,
  hinthead = Tips
}

\setRecipeLengths{pictureheight=4cm}

\renewcommand*{\recipesection}[2][]{\subsection[#1]{#2}}

% section %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\setcounter{secnumdepth}{1}
\renewcommand{\thesection}{\Alph{section}}

% recipe commands %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newcommand{\recipepicture}[1]{pictures/#1}
\newcommand{\includerecipe}[1]{\include{recipes/#1}}

\newcommand{\recipesektion}[1]{\multicolumn{2}{l}{\rule{0em}{1.4em}\textbf{\textsc{[#1]}}}}
\newcommand{\optional}{\recipesektion{valfritt}}
\newcommand{\shortstep}{\vspace{\baselineskip}}

% commands %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newcommand{\unsure}[1]{\textit{\textcolor{orange}{#1}}}

% document %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{document}

\maketitle
\thispagestyle{empty}
\newpage
\tableofcontents
\thispagestyle{empty}
\newpage

\titleformat{\section}[display]{\normalfont\Large\filcenter\bfseries}{Avsnitt~\thesection}{1em}{\Huge}
